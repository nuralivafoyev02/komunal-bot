...
function getMonthlyTotals(userId, months = 6) {
  const payments = PaymentRepo.findByUser(userId).filter(p => p.type === 'topup');
  const result   = {};
...
module.exports = { getMonthlyTotals, compareMonths, topKomunal, allTimeTotals, averageMonthly, generateInsight };
